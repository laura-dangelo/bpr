...
MH_p50_n25 <- list()
tmp = list()
load("01_MH_Gaussian/1_tuning/dist_best_MH_p50_n25.Rdata")
seeds = 1:50
...
for(seed in seeds)
{
  ...
  run <- sample_bpr( y ~ X-1, data = data.frame(y,X),
                     ...
                     burnin = 5000,
                     pars = list(max_dist = 1, max_dist_burnin = 1e+6),
                     seed = 1,
                     verbose = FALSE)
  end <- Sys.time()
  time <- end - start
...
save(MH_p50_n25, file = filenamesave)

lapply(MH_p50_n25, function(x) unlist(x$effSize) )

### n = 50
MH_p50_n50 <- list()
tmp = list()
load("01_MH_Gaussian/1_tuning/dist_best_MH_p50_n50.Rdata")
seeds = 1:50
...
for(seed in seeds)
{
  ...
  run <- sample_bpr( y ~ X-1, data = data.frame(y,X),
                     iter = nrep, perc_burnin = 0.5,
                     pars = list(max_dist = 100, max_dist_burnin = 20000),
                     # seed = 1,
                     state = rep(0,50),
                     verbose = FALSE)
  end <- Sys.time()
...
filenamesave = paste0("01_MH_Gaussian/2_time_effsize/MH_toess_p", p, "_n", n, ".Rdata")
save(MH_p50_n50, file = filenamesave)
lapply(MH_p50_n50, function(x) unlist(mean(x$effSize) ) )

### n = 100
MH_p50_n100 <- list()
tmp = list()
load("01_MH_Gaussian/1_tuning/dist_best_MH_p50_n100.Rdata")
seeds = 1:50
...
for(seed in seeds)
{
  ...
  run <- sample_bpr( y ~ X-1, data = data.frame(y,X),
                     iter = nrep, perc_burnin = 0.5,
                     burnin = 5000,
                     pars = list(max_dist = 50, max_dist_burnin = 2000),
                     state=rep(0,50),
                     # seed = 1,
                     verbose = FALSE)
  end <- Sys.time()
  ...
  tmp$time <- time
  tmp$effSize <- effSize
  MH_p50_n100[[seed]] <- tmp
  
  if(seed%%10==0) print(seed)
}

filenamesave = paste0("01_MH_Gaussian/2_time_effsize/MH_toess_p", p, "_n", n, ".Rdata")
save(MH_p50_n100, file = filenamesave)
lapply(MH_p50_n100, function(x) unlist(mean(x$effSize) ) )

### n = 200
rm(list=ls())
MH_p50_n200 <- list()
tmp = list()
load("01_MH_Gaussian/1_tuning/dist_best_MH_p50_n200.Rdata")
seeds = 1:50
...
for(seed in seeds)
{
  ...
  tmp$time <- time
  tmp$effSize <- effSize
  MH_p50_n200[[seed]] <- tmp
  
  if(seed%%10==0) print(seed)
}

filenamesave = paste0("01_MH_Gaussian/2_time_effsize/MH_toess_p", p, "_n", n, ".Rdata")
save(MH_p50_n200, file = filenamesave)
