...
for(seed in seeds)
{
  ...
  tmp$run <- cbind(run@sim$samples[[1]]$`beta[1]`, run@sim$samples[[1]]$`beta[2]`,
                   ...
                   run@sim$samples[[1]]$`beta[5]`) ### modificare al variare di p
  effSize <- apply(tmp$run[-burnin,], 2, effectiveSize)
  tmp$time <- time
  tmp$effSize <- effSize
  stan_p5_n25[[seed]] <- tmp
...
save(stan_p5_n25, file = filenamesave)
...
for(seed in seeds)
{
  ...
  tmp$time <- time
  tmp$effSize <- effSize
  stan_p5_n50[[seed]] <- tmp
  
  if(seed%%10==0) print(seed)
}

filenamesave = paste0("01_MH_Gaussian/2_time_effsize/stan_toess_p", p, "_n", n, ".Rdata")
save(stan_p5_n50, file = filenamesave)
...
for(seed in seeds)
{
  ...
  tmp$time <- time
  tmp$effSize <- effSize
  stan_p5_n100[[seed]] <- tmp
  
  if(seed%%10==0) print(seed)
}

filenamesave = paste0("01_MH_Gaussian/2_time_effsize/stan_toess_p", p, "_n", n, ".Rdata")
save(stan_p5_n100, file = filenamesave)
...
for(seed in seeds)
{
  ...
  tmp$time <- time
  tmp$effSize <- effSize
  stan_p5_n200[[seed]] <- tmp
  
  if(seed%%10==0) print(seed)
}

filenamesave = paste0("01_MH_Gaussian/2_time_effsize/stan_toess_p", p, "_n", n, ".Rdata")
save(stan_p5_n200, file = filenamesave)
