...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    w = w / mean(w)
    effic_sim_is_p5_n25[[d]] <- w
    
  }

  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_is_p5_n25, effectiveSize  )) ) 
...
save(best_dist, 
     file="03_IS_Gaussian/1_tuning/dist_best_IS_p5_n25.Rdata")
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    w = exp( run$sim$logw[-burnin] - mean(run$sim$logw[-burnin]) )
    w = w / mean(w)
    effic_sim_is_p5_n50[[d]] <- w
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_is_p5_n50, effectiveSize  )) ) 
...
save(best_dist, 
     file="03_IS_Gaussian/1_tuning/dist_best_IS_p5_n50.Rdata")
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    w = exp( run$sim$logw[-burnin] - mean(run$sim$logw[-burnin]) )
    w = w / mean(w)
    effic_sim_is_p5_n100[[d]] <- w
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_is_p5_n100, effectiveSize  )) ) 
  if(var(df$value) == 0) print(seed)
  best_dist[seed] = as.numeric( attr(which.max(tapply(df$value, df$dist, mean)), "names") )
...
save(best_dist, 
     file="03_IS_Gaussian/1_tuning/dist_best_IS_p5_n100.Rdata")
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    w = exp( run$sim$logw[-burnin] - mean(run$sim$logw[-burnin]) )
    w = w / mean(w)
    effic_sim_is_p5_n200[[d]] <- w
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_is_p5_n200, effectiveSize  )) ) 
  if(var(df$value) == 0) print(seed)
  best_dist[seed] = as.numeric( attr(which.max(tapply(df$value, df$dist, mean)), "names") )
...
save(best_dist, 
     file="03_IS_Gaussian/1_tuning/dist_best_IS_p5_n200.Rdata")
