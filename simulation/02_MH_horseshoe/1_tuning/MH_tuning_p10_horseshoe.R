...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    effic_sim_mh_p10_n25[[d]] <- run$sim$beta[-burnin,]
    
  }

  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_mh_p10_n25, function(x) mean(apply(x, 2, effectiveSize ) )) ) )
...
save(best_dist, 
     file="02_MH_horseshoe/1_tuning/dist_best_MH_p10_n25.Rdata")

rm(effic_sim_mh_p10_n25)
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    effic_sim_mh_p10_n50[[d]] <- run$sim$beta[-burnin,]
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_mh_p10_n50, function(x) mean(apply(x, 2, effectiveSize ) )) ) )
...
save(best_dist, 
     file="02_MH_horseshoe/1_tuning/dist_best_MH_p10_n50.Rdata")

rm(effic_sim_mh_p10_n50)
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    effic_sim_mh_p10_n100[[d]] <- run$sim$beta[-burnin,]
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_mh_p10_n100, function(x) mean(apply(x, 2, effectiveSize ) )) ) )
...
save(best_dist, 
     file="02_MH_horseshoe/1_tuning/dist_best_MH_p10_n100.Rdata")

rm(effic_sim_mh_p10_n100)
...
for(seed in 1:50)
{
  ...
  for(d in 1:length(dist_seq))
  {
    ...
    effic_sim_mh_p10_n200[[d]] <- run$sim$beta[-burnin,]
    
  }
  
  df = data.frame(dist = dist_seq, 
                  value = unlist(lapply(effic_sim_mh_p10_n200, function(x) mean(apply(x, 2, effectiveSize ) )) ) )
...
save(best_dist, 
     file="02_MH_horseshoe/1_tuning/dist_best_MH_p10_n200.Rdata")

rm(effic_sim_mh_p10_n200)